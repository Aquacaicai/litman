...
const authorChartOptions = computed(() => {
  ...
  const topAuthors = [...authorStats.value].sort((a, b) => a.count - b.count)
...
const filteredAuthors = computed(() => {
  ...
  return authorStats.value.filter((stat) => stat.author.toLowerCase().includes(searchTerm))
})
...
async function loadKeywordTrends() {
  ...
  try {
    ...
    if (keywordsStats.value && Object.keys(keywordsStats.value).length > 0) {
      const years = Object.keys(keywordsStats.value)
        .map(Number)
        .sort((a, b) => b - a)
...
const availableYears = computed(() => {
  ...
  return Object.keys(keywordsStats.value)
    .map(Number)
    .sort((a, b) => a - b)
})
...
<template>
  <div>
    ...
    <div class="tabs tabs-boxed mb-6">
      <a
        class="tab"
        :class="{ 'tab-active': activeTab === 'authorStats' }"
        @click="activeTab = 'authorStats'"
        >Author Statistics</a
      >
      <a
        class="tab"
        :class="{ 'tab-active': activeTab === 'keywordStats' }"
        @click="activeTab = 'keywordStats'"
        >Keyword Trends</a
      >
    </div>
        ...
        <div v-else-if="authorStats" class="space-y-4">
          ...
          <div class="overflow-x-auto">
            <table class="table">
              ...
              <tbody>
                <tr v-for="[index, stat] in paginatedAuthors.entries()" :key="index">
                  <td>{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td>
                  <td>{{ stat.author }}</td>
                  <td>{{ stat.count }}</td>
                  <td>
                    <button class="btn btn-xs btn-ghost" @click="viewAuthorPapers(stat.author)">
                      View Papers
                    </button>
                  </td>
                </tr>
    ...
    <div v-if="activeTab === 'keywordStats'" class="card bg-base-100 shadow-xl">
      <div class="card-body">
        ...
        <div v-else-if="keywordsStats && selectedYear" class="overflow-x-auto">
                ...
                <td>
                  <template v-if="Number(selectedYear) > Math.min(...availableYears)">
                    <template v-if="keywordsStats[Number(selectedYear) - 1]">
                      <template
                        v-if="
                          keywordsStats[Number(selectedYear) - 1].find(
          ...
          <!-- Keyword trend chart -->
          <div
            v-if="keywordsStats && Object.keys(keywordsStats).length > 0"
            class="w-full h-80 mt-6"
          >
