...
def import_split_xml_files(target_dir, api_url='http://localhost:2747/api/import/xml', batch_size=None):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if result.get('success'):
                        ...
                        tqdm.write(
                            f"Successfully imported {xml_file}: {imported_count} articles")
                    else:
                        fail_count += 1
                        tqdm.write(
                            f"Failed to import {xml_file}: {result.get('message')}")
                else:
                    fail_count += 1
                    tqdm.write(
                        f"Error importing {xml_file}: HTTP {response.status_code}")
...
if __name__ == "__main__":
    ...
    num_files = split_large_xml("dblp.xml", split_dir, target_size_mb=100)

    import_result = import_split_xml_files(split_dir)
